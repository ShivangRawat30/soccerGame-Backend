...
use actix_web::{body, delete, get, post, put, web, HttpResponse, Responder};
use chrono::Utc;
use serde_json::json;
use crate::models::games::{GameModel, CreateGameSchema, UpdateGameSchema};
use crate::AppState;
...
#[put("/games/game/{id}")]
async fn updated_game(path: web::Path<uuid::Uuid>, data: web::Data<AppState>, body: web::Json<UpdateGameSchema>) -> impl Responder {
    let game_id = path.into_inner();
    let query_result = sqlx::query_as!(GameModel, "SELECT * FROM games where id = $1", game_id)
        .fetch_one(&data.db)
        .await;
